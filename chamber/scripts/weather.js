...
async function currentWeatherAPIFetch() {
  ...
  function displayResults(data) {
    ...
    highTemp.innerHTML =  `High Temperature: <span class = "label">${Math.ceil(data.main.temp_max)}&degF</span>`;
    lowTemp.innerHTML =  `Low Temperature: <span class = "label">${Math.ceil(data.main.temp_min)}&degF</span>`;
    humidity.innerHTML = `<span>🌡💧</span>Humidity: <span class = "label">${data.main.humidity}%</span>`;
    sunrise.innerHTML = `Sunrise: <span class = "label">${sunriseTime}</span>`;
    sunset.innerHTML = `Sunset: <span class = "label">${sunsetTime}</span>`;
...
function showResults(data) {
    ...
    firstForecast.innerHTML = `${firstDayName}: <span class = "label">${Math.ceil(firstDayForecast.main.temp)}&deg;F</span>`;
    secondForecast.innerHTML = `${thirdDayName}: <span class = "label">${Math.ceil(thirdDayForecast.main.temp)}&deg;F</span>`;
    thirdForecast.innerHTML = `${sixDayName}: <span class = "label">${Math.ceil(sixDayForecast.main.temp)}&deg;F</span>`;
}
